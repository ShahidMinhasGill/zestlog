...
@endsection
@push('after-styles')
    <link src="{{ asset('css/sweetalert.css') }}">
    <style>
        .disabledTab{
            pointer-events: none;
...
@section('content')
    <div class="page-content">
        ...
        @include('client.plan.partials._main-section')
        ...
        <div class="card">
            <div class="card-body">
                ...
                @include('client.plan.partials._training-plan-popup')
                ...
                <div id="plan-exercise-video"></div>
            </div>
        </div>
    </div>
    </div>
...
@push('after-scripts')
    ...
    <script>
        ...
        $RedirectRoute = '{{ URL::route('plans.index') }}';
        $addDragdropRoute = '{{ URL::route('plan.drag.drop.exercise') }}';
        $addCommentRoute = '{{ URL::route('plan.add.comment') }}';
        $weeklyTrainingSetupRoute = '{{ URL::route('plan-weekly-training-setup') }}';
        ...
        $importWeekPlanDataRout = '{{ URL::route('import.plan.week') }}';
        $deleteDraftRoute = '{{ URL::route('delete.draft.plan') }}';
        $dayPlanSave = '{{ URL::route('day.plan.save') }}';
        $oneDayPlanSave = '{{ URL::route('one.day.plan.save') }}';
        ...
        $trainingDayIds = $.parseJSON('{!! $data['trainingDayIds'] !!}');
        ...
        $(document).ready(function () {
            ...
            $('.training-active-days').select2({});
            setTimeout(function () {
                $('.js-example-basic-multiple').val($equipmentIds).change();
            }, 500);
            setTimeout(function () {
                $('.training-active-days').val($trainingDayIds).change();
            }, 500);
            ...
            $('body').on('click', '.delete_training_plan', function (e) {
                let clickedId = this.id.split('_');
            ...
            $('body').on('click', '#save-training-plan, #save-close-training-plan', function (e) {
                ...
                // $('#training_day_id').val($daysCount);
                $('#training_day_id').val([$daysCount]).change();
                // $("#training_day_id").val($daysCount).attr("selected","selected");
                saveOverview(close);
            });
            ...
            $('body').on('click', '.week-plan', function (e) {
                let targetId = e.target.id;
                if (!($('#' + targetId).hasClass('day-plan-save-popup')) && !($('#' + targetId).hasClass('plan-import-one-day-class'))) {
            ...
            //Import one day plan from program database
            $('body').on('click', '#import-database', function () {
                ...
                if (count != 1) {
                    swal("Error!", 'Please select one plan!', "error");
                    return false;
                }
                $('input.checkbox-import-select:checkbox:checked').each(function () {
                    $id = $(this).attr("id");
        ...
        var cal = {
            ...
            overViewPopUp: function (tabName) {
                $.get('/plans-overview/' + $id + '/'+ $isEdit, {tabName: tabName}, function (response) {
        ...
        var saveOverview = function (close) {
            ...
            formData.push({name: 'training_day_id', value: $daysCount});
            formData.push({name: 'is_edit', value: $isEdit});

            let activeTab = $('.active_tab').val();
            $.post('/plans-overview', formData, function (response) {

    ...
    {{--<div class="card">--}}
        {{--<div class="card-body">--}}
            ...
            {{--<div class="tab-content" id="nav-tabContent">--}}
                    ...
                    {{--@include('client.partials._clients-waiting-tabcontent')--}}
            {{--</div>--}}
        {{--</div>--}}
    {{--</div>--}}
        ...
        <div class="row">
                <div class="col-12 d-flex align-items-center">
                    ...
                    <div class="d-flex align-items-center">
                            <div class="header-avtar" id="profile_pic">
           ...
           {{-- @include('client.partials._clients-pro-head') --}}
                     ...
                     <nav>
                        <div class="nav nav-pills" id="nav-tab" role="tablist">
                            ...
                            <button class="btn grey-outline-btn ml-auto booking-popup-info"id="booking-{{$data['unique_id']}}">Booking</button>
                        </div>
                    </nav>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="col-12 clearfix mb-3">
                                        <div class="float-left">
                                            {{--<button class="btn success-btn" id="import-predevelop-plan" data-toggle="modal" data-target="#week-import-client">Import</button>--}}
                                            ...
                                            <div class="float-right published-div"id="btn-publish-id" style="margin-left: 20px;">
                                                <a class="btn primary-btn  mr-2" id="is_publish_plan" data-toggle="modal" data-target="#plan-publish-modal" style="color: white"></a>
                                            </div>
                                        ...
                                        @include('client.partials._week-import-client-popup')
...
@push('after-scripts')
    ...
    <script type="text/javascript">
        ...
        $getExercisesRoute = '{{ URL::route('client-get-exercises') }}';
        ...
        $importPlanDataRout = '{{ URL::route('client.plan.get.training.programs') }}';
        $bookingInformationRout = '{{ URL::route('user.booking.information') }}';
        $dayPlanSave = '{{ URL::route('client.day.plan.save') }}';
        $oneDayPlanSave = '{{ URL::route('client.one.day.plan.save') }}';
        ...
        $(document).ready(function () {
            ...
            var clickedUniqueId = getUrl.pathname.split('/')[2];


            clientTrainingProgram(clickedUniqueId);
            $('#search').val('');
            $(document).on("click", '.exercises.paq-pager ul.pagination a', function (e) {
                e.preventDefault();
                $page = $(this).attr('href').split('page=')[1];
                getExercisesData();
            });
            $(document).on("click", '.import-plan-class.paq-pager ul.pagination a', function (e) {
                e.preventDefault();
                $page = $(this).attr('href').split('page=')[1];
                $plan_type = 1;
                importPlans();
            ...
            $('body').on('click', '#import-database', function () {
                var importPlanId = '';
                var count = 0;
                if ($('input[name=radio_import_plans]:checked').length < 1) {
                    swal("Error!", 'Please select plan!', "error");
                    return false;
                }
                $id = $('input[name="radio_import_plans"]:checked').attr('id')
                ...
                $.ajax({
                    ...
                    data: $formData,
                    success: function (data) {
                        swal("Success!", 'The selected training plan was imported successfully.', "success");
            ...
            $('body').on('click', '.day-plan-save-popup', function () {
                $selectedDayId = this.id.split('_')[1];
                $formData = {
                    '_token': $token,
                    unique_id: $unique_id,
                    dayId: $selectedDayId
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('.one-day-save').select2({});
                        let equipmentIds = $.parseJSON(data.equipmentIds);
                        setTimeout(function () {
                            $('.one-day-save').val(equipmentIds).change();
                        }, 500);
                    },
                    error: function ($error) {
            ...
            $('body').on('click', '#save-one-day-plan', function () {
                $title = $('.title-fileds').val()
                $goal = $('.plan-creator-goal').val()
                if ($title != '' && $goal != '') {
                    ...
                    swal("Error!", 'Please Fill this field info!', "error");
                }
            });
            $('body').on('click', '.cancel-plan-draft', function () {
                $.fancybox.close();
            });
            ...
            $('body').on('click', '#is_publish_plan', function () {
                ...
                if ($isPublishPlan == 'Published')
                    $('#publish-client-plan').html('Unpublish');
                else
                    $('#publish-client-plan').html('Publish');
            });
            $('body').on('click', '.booking-popup-info', function () {
                $id  = this.id;
                $id = $id.split('-')[1];
                $formData = {
                    '_token': $token,
        ...
        var clientTrainingProgram = function (clickedUniqueId) {
                $unique_id = clickedUniqueId;
                let id = $unique_id.split('_')[1];
                $(".client-plan-information-div").show();
                $("#user-profile").show();
                $formData = {
                    '_token': $token,
                    id: id,
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success == true) {
                            ...
                            $endProgramDate = response.data.end_program_date;
                            ...
                            $(".enable").css('color', 'black');
                            $("#"+slectWeek).css('color', 'blue');
                            $(".enable").css('font-weight', 'bold');
                            ...
                            $.each(response.data, function (index, value) {
                                if(index != 'profile_pic') {
                                    ...
                                    $('#profile_pic').html('<img src="'+value+'"  class="mr-2" alt="profile-pic.png">');
                                }
                            });
                        }
                        getWeeklyPlanSetup();
        ...
        var cal = {
            ...
            overViewPopUp: function (tabName) {
                $.get('/client-plans-overview/' + $id+'-'+$unique_id, {tabName: tabName}, function (response) {

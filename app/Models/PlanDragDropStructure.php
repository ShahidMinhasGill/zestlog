...
{
    protected $fillable = ['plan_id', 'day_id', 'structure_id', 'exercise_id', 'workout_counter', 'workout_sub_counter', 'workout_type', 'workout_set_type_id', 'position', 'position_id', 'set_id', 'rep_id', 'duration_id', 'note_id', 'rm_id', 'tempo_id', 'rest_id', 'form_id', 'stage_id', 'wr_id','is_delete','is_edit'];
    ...
    */
    ...
    {
        $sql = \DB::table('plan_drag_drop_structures as pd');
        ...
        $sql->where('pd.plan_id', '=', $params['plan_id']);
    ...
    {
        $sql = \DB::table('plan_drag_drop_structures as pd');
        $sql->select('pd.id as training_plan_id', 'pd.day_id','pd.structure_id', 'tps.name as structure_name', 'pd.workout_type', 'e.name', 'pd.set_id', 'pd.rep_id', 'pd.rm_id', 'pd.duration_id'
            ,'pd.note_id', 'pd.tempo_id', 'pd.rest_id', 'pd.form_id', 'pd.stage_id', 'pd.wr_id','e.id as exercise_id', 'pd.position as drop_position', 'e.male_illustration', 'e.female_illustration'
        );
        $sql->join('exercises as e', 'e.id', '=', 'pd.exercise_id');
        $sql->join('training_plan_structures as tps', 'tps.id', '=', 'pd.structure_id');
        $sql->where('pd.plan_id', '=', $params['plan_id']);
        if (!empty($params['is_main_workout'])) {
            $sql->addSelect('wts.name as set_type', 'wts.key_value', 'pd.workout_set_type_id', 'pd.workout_counter', 'pd.workout_sub_counter', 'pwtsp.position as drop_position_main', 'pwtsp.id');
            $sql->join('workout_type_sets as wts', 'wts.id', '=', 'pd.workout_set_type_id');
            $sql->join('plan_week_training_setup_positions as pwtsp', function($join) {
                $join->on('pwtsp.id', '=', 'pd.position_id');
                $join->on('pwtsp.workout_type_set_id', '=', 'pd.workout_set_type_id');
            });
            $sql->whereIn('pd.structure_id', $params['structure_id']);
            $sql->orderBy('pwtsp.position', 'asc');
        } else {
            $sql->addSelect('tts.position as drop_position_main', 'tts.id');
            $sql->join('temp_training_setups as tts', function($join) {
                $join->on('tts.id', '=', 'pd.position_id');
                $join->where('pd.workout_set_type_id', '=', 0);

            });
            $sql->where('pd.structure_id', '!=', 2);
    ...
    {
        $tableName = 'plan_drag_drop_structures';
        if (!empty($params['isEdit'])) {
            $tableName = 'draft_plan_drag_drop_structures';
        }
        $sql = \DB::table($tableName . ' as pd');
        $sql->select('pd.id', 'e.name', 'pd.set_id', 'pd.rep_id', 'pd.duration_id', 'pd.note_id', 'pd.rm_id', 'pd.tempo_id', 'pd.rest_id', 'pd.form_id', 'pd.stage_id', 'pd.wr_id', 'e.male_illustration');
        $sql->where('pd.plan_id', '=', $params['planId']);
    ...
    {
        $sql = \DB::table('plan_drag_drop_structures as pds');
        $sql->selectRaw('Distinct(eq.id)');
        $sql->where('pds.plan_id', '=', $params['planId']);
        $sql->where('pds.day_id', '=', $params['dayId']);
        $sql->join('exercises as e', 'e.id', '=', 'pds.exercise_id');
